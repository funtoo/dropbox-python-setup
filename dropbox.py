...
# Copyright 2017 Funtoo Solutions, Inc.
#
# dropbox
...
WARNING = u"In order to use Dropbox, you must initialize Dropbox for this user. Click 'OK' to set up Dropbox for this user, or 'Cancel' to cancel."
WARNING_CLI = u"In order to use Dropbox, you must initialize Dropbox for this user. Would you like to do this?"
UNPACKING = u"Unpacking Dropbox... %d%%"
INSTALLED = "Dropbox is now installed. Type 'dropbox start' (or add this to startup scripts) to start Dropbox."

PARENT_DIR = os.path.expanduser("~")
DIST_DIR = "%s/.dropbox-dist" % PARENT_DIR
DROPBOXD_PATH = "%s/dropboxd" % DIST_DIR
...
def user_setup():
    local_fn = "/usr/share/dropbox/dropbox-dist.tar.gz"
    with open(local_fn,"rb") as local_file:
        archive = tarfile.open(fileobj=local_file, mode='r:gz')
        ...
        archive.close()
        lnx_path = glob.glob("%s/dropbox-lnx.*" % DIST_DIR)
        if not len(lnx_path):
            print("Error -- could not find dropbox installation.")
            sys.exit(1)
        lnx_path = lnx_path[0]
        # stuff we don't want from the distribution archive.
        for zap_pat in [ "libpopt.so.0", "libdrm.so.2", "libffi.so.6", "libGL.so.1", "libX11*" ]:
            zap_results = glob.glob("%s/%s" % (lnx_path, zap_pat))
            for zap_file in zap_results:
...
if GUI_AVAILABLE:
        ...
        class DownloadDialog(gtk.Dialog):
            ...
            def unpack_dropbox(self):
                ...
                def finished():
                    ...
                    self.ok.show()
                    self.label.set_markup(INSTALLED)
                    self.label.show()
                ...
                self.task = GeneratorTask(user_setup,
                                          unpack_progress,
            ...
            def handle_ok(self, button):
                if not self.installed:
                    ...
                    self.cancel.hide()
                    self.progress.show()
                    self.label.set_markup("Installing Dropbox...")
                    self.unpack_dropbox()
                    self.installed = True
                else:
                    gtk.main_quit()
    ...
    def download():
        ...
        if not sys.stdout.isatty():
            sys.stdout.write("Please run dropbox start interactively to set up dropbox.")
        else:
            if yes_no_question(WARNING_CLI):
                sys.stdout.write("Beginning Dropbox installation...\n")
                for foo in user_setup():
                    sys.stdout.write(".")
                    sys.stdout.flush()
                sys.stdout.write("\n" + INSTALLED + "\n\n")
...
"""
    ...
    if not start_dropbox():
        if not should_install:
            console_print()
            console_print(u"Dropbox has not yet been set up.")
            console_print(u"Run \"dropbox start -i\" to set up dropbox for this user.")
            return
        # install dropbox!!!
        download()
